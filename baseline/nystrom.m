function [cluster_labels,sampling_time, eigen_time, contruct_time, time_kmeans] = nystrom(data, num_samples, sigma, num_clusters)
...
A = exp(-(A.*A) ./ (2*sigma*sigma));
B = exp(-(B.*B) ./ (2*sigma*sigma));
...
clear sq_sum V;
cluster_labels = litekmeans(U,num_clusters,'MaxIter',50,'Replicates',1);
% Restore cluster_labels in original order
cluster_labels(permed_index) = cluster_labels;
time_kmeans = toc;
clear permed_index U;
...
%
two_ab = zeros(size(aa, 2), size(bb, 2));
...
for i = 1:num_iter
  ...
  abtmp = full(a'*b(:, start_index:end_index));
